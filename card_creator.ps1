...
foreach ($SUIT in $CARDSUITS) {
    ...
    switch ($SUIT) {
        ...
        "Spades" {
            $SUITPATH="<path d='M 7.989,3.103 C 7.747,-0.954 0.242,-8.59 0,-10.5 c -0.242,1.909 -7.747,9.545 -7.989,13.603 -0.169,2.868 1.695,4.057 3.39,4.057 1.8351685,-0.021581 3.3508701,-2.8006944 3.873,-3.341 0.242,0.716 -1.603,6.682 -2.179,6.682 l 5.811,0 C 2.33,10.501 0.485,4.535 0.727,3.819 1.1841472,4.3152961 2.5241276,7.0768295 4.601,7.16 6.295,7.159 8.158,5.971 7.989,3.103 z' />"
        ...
        "Hearts" {
            $SUITPATH="<path style='fill:#df0000' d='M 3.676,-9 C 0.433,-9 0,-5.523 0,-5.523 0,-5.523 -0.433,-9 -3.676,-9 -5.946,-9 -8,-7.441 -8,-4.5 -8,-0.614 -1.4208493,3.2938141 0,9 1.35201,3.2985969 8,-0.614 8,-4.5 8,-7.441 5.946,-9 3.676,-9 z' />"
    ...
    foreach ($FACEVAL in $CARDFACEVALS) {
        if (-Not ($q -or $quiet)) { echo "Creating $FACEVAL of $SUIT" }
        $OUTFILE="$o\$FACEVAL$($SUIT.Substring(0,1)).svg"
        ...
        switch ($FACEVAL) {
            ...
            "4" {
                echo "<g transform='matrix($SUITSCALE,0,0,$SUITSCALE,$SUITX1,$SUITY2)'>$SUITPATH</g>" >> "$OUTFILE"
            ...
            "7" {
                echo "<g transform='matrix($SUITSCALE,0,0,$SUITSCALE,$SUITX1,$SUITY1)'>$SUITPATH</g>" >> "$OUTFILE"
            ...
            "A" {
                echo "<g transform='matrix($ACESCALE,0,0,$ACESCALE,$ACEX,$ACEY)'>$SUITPATH</g>" >> "$OUTFILE"
            }
            "J" {
                echoRoyalItems >> "$OUTFILE"
    ...
    if (-Not ($q -or $quiet)) { echo "--------------------------------------" "Done with $SUIT" }
}
if (-Not ($q -or $quiet)) { echo "--------------------------------------" "All finished - Enjoy" "--------------------------------------" }